...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase.from('awroad_news').select('*').eq('id', id).single();
    ...
    <main style={{ position: 'relative', minHeight: '100vh', background: '#070A14', overflow: 'hidden' }}>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '160px 0 100px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '0 28px' }} dir={isRtl ? 'rtl' : 'ltr'}>
          <Link href={`/${locale}/news`} style={{ fontSize: 13, color: '#A78BFA', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 32, fontFamily: isRtl ? 'Cairo, sans-serif' : 'Inter, sans-serif' }}>
            {isRtl ? 'الأخبار ←' : '← Back to News'}
          </Link>
